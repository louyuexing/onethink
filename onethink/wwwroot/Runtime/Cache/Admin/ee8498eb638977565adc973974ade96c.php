		...
		<table class="">
			...
			<?php foreach($activeall as $row):?>
					<tr>
						...
						<td><span><?php echo (time_format($row['acreate_time'])); ?></span></td>
						<td><span><?php echo (time_format($row['stop_time'])); ?></span></td>
						<td class="pass"><?php if($row['activestatus']==0){echo '未通过';}else{echo '通过';}?></td>
						<td><?php if(($vo["status"]) == "1"): else: ?>
							<a href="javascript:void(0)" class="status"><?php if($row['activestatus']==0){echo '通过';}else{echo '取消通过';}?></a>
							<input type="hidden" name="status" value="<?php echo ($row['aid']); ?>"/><?php endif; ?>
	...
	<script type="text/javascript">
        ...
        $('.status').click(function(){
          ...
          var text=$(this).closest('tr').find('.pass');
           var a=$(this);
           console.debug(a);
          $.getJSON('<?php echo U('Property/activestatus');?>',{'re':re},function(e){
               if(e==1){
                   $(text).text('通过');
                   $(a).text('取消通过');
               }else{
        ...
        $('.del').click(function(){
            if(confirm('你确定要删除吗')){
                ...
                var a=$(this);
                var y='';
                $.getJSON('<?php echo U('Property/activedel');?>',{'del':del},function(e){
                    if(e==1){
                        $(a).closest('tr').remove()
					}
        ...
        //导航高亮
        highlight_subnav('<?php echo U('Property/active');?>');
	</script>
